...
$stmt = $pdo->prepare("
    ...
    WHERE p.client_id = :client_id AND p.agency_id = :agency_id AND p.deleted_at IS NULL
");
$stmt->execute(['client_id' => $client_id, 'agency_id' => $agency_id]);
...
$stmt = $pdo->prepare("SELECT id, project_name, status, project_type FROM projects WHERE client_id = :client_id AND agency_id = :agency_id AND deleted_at IS NULL ORDER BY created_at DESC");
$stmt->execute(['client_id' => $client_id, 'agency_id' => $agency_id]);
...
<div class="px-4 py-5 sm:px-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="client-edit.php?id=<?php echo $client_id; ?>" class="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-full hover:bg-indigo-100 transition shadow-sm text-sm font-medium">Edit Client</a>
    </div>
</div>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-6">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
                ...
                <div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900"><?php echo htmlspecialchars($client['gst_number'] ?: '-'); ?></dd>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <a href="project-create.php?client_id=<?php echo $client_id; ?>" class="text-sm text-indigo-600 hover:text-indigo-900 font-medium">Add Project</a>
            </div>
