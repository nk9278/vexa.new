...
// Employees can only see clients linked to projects they are assigned to
$query = "
    SELECT DISTINCT c.*
    FROM clients c
    ...
    AND pa.employee_id = :employee_id
...
if ($search) {
    $query .= " AND (c.client_name LIKE :search OR c.company_name LIKE :search)";
...
<div class="px-4 py-5 sm:px-6 flex justify-between items-center">
    <h1 class="text-3xl font-bold leading-tight text-gray-900">My Clients</h1>
</div>

<div class="px-4 sm:px-6 mb-6">
    <form method="GET" action="clients.php" class="flex flex-col sm:flex-row gap-4">
        <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search name or company..." class="px-4 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500 w-full sm:w-64 outline-none">
                    ...
                    <?php foreach ($clients as $client): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <?php echo htmlspecialchars($client['city'] ?: '-'); ?><?php echo $client['country'] ? ', ' . htmlspecialchars($client['country']) : ''; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="3" class="px-6 py-4 text-center text-sm text-gray-500">No clients assigned to your projects.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
