    ...
    if (!verifyCsrfToken($csrf_token)) {
        ...
        if ($error) {
            ...
            try {
                ...
                $emp_id = $submission['employee_id'];
                $task_name_esc = htmlspecialchars($submission['task_name'], ENT_QUOTES);
                if ($action === 'Approve') {
                    createNotification($emp_id, 'Submission Approved', "Your submission for task: $task_name_esc was approved.", "/employee/task-view.php?id=" . $submission['task_id']);
                } else {
                    createNotification($emp_id, 'Revision Required', "Your submission for task: $task_name_esc requires revision.", "/employee/task-view.php?id=" . $submission['task_id']);
                }
